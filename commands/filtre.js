...
module.exports = {
  ...
  run: async (client, interaction) => {
    try {
      ...
      .setDescription('** Ritmi Keşfet, Ses Sihirlerini Aşağıdan Seç!**')
...
if(!filters?.includes(button?.customId)) return
      ...
      if (!filtre) return interaction?.editReply({ content: '❌ Geçersiz İsim', ephemeral: true }).catch(e => { })
     filtre = filtre?.toLowerCase()
   
      if (filters?.includes(filtre?.toLowerCase())) {
        if (queue?.filters?.has(filtre)) {
          queue?.filters?.remove(filtre)
          embed?.setDescription(`Sihir : **{filter}**, Uygulanma Durumu: **{status}**`.replace("{filter}", filtre).replace("{status}", "❌"))
        ...
        embed?.setDescription(`❌ Filtre bulunamadı!!`.replace("{filters}", filters?.map(mr => `\`${mr}\``).join(", ")))
